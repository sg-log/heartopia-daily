...
# 3) X raw media can contain quoted/replied/event/map images. For X, the verified
#    official embed screenshot is the only deterministic-review input unless media
#    attribution is proven separately. Generic web sources keep their captured media.
replace_once('tools/weather-deterministic-review.mjs',
...
# 4) Text may supplement only a start slot already supported by visible time labels.
#    It can never turn an arbitrary image into a weather result.
...
# 6) Weekly X review follows the same rule: ambiguous raw media is not authoritative;
#    the verified embed screenshot remains eligible. Web/Instagram/TikTok candidates
#    use their own captured evidence through the generic adapter.
...
# 7) Regression: text-assisted recovery needs visual time-label evidence; X unscoped
#    media fails closed.
