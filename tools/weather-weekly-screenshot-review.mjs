...
async function inspectScreenshot(page, imageDataUrl, templates) {
  return page.evaluate(async ({ imageDataUrl, templates }) => {
    ...
    // Detect the large blue weather header. Original raw media is preferred; embed screenshots remain a fallback.
    const step=4, gw=Math.ceil(full.width/step), gh=Math.ceil(full.height/step), grid=new Uint8Array(gw*gh);
    for(let gy=Math.floor(gh*.20);gy<gh;gy++) for(let gx=0;gx<gw;gx++){
      let hits=0,total=0;
      for(let yy=gy*step;yy<Math.min(full.height,(gy+1)*step);yy+=2) for(let xx=gx*step;xx<Math.min(full.width,(gx+1)*step);xx+=2){
        ...
        if(b>135&&g>100&&b>r*1.08&&hsv.s>.18&&hsv.h>=168&&hsv.h<=242) hits++;
      }
      if(hits>=Math.max(1,Math.ceil(total*.30))) grid[gy*gw+gx]=1;
    }
    ...
    for(let gy=0;gy<gh;gy++)for(let gx=0;gx<gw;gx++){
      ...
      if(box.w>=70&&box.h>=40&&box.y>=full.height*.18&&box.w/box.h>=1.0&&box.w/box.h<=2.8)components.push(box);
    }
    ...
    // A real in-game weekly forecast has a large white/light-neutral list directly below the blue header.
    const panelLeft=Math.max(0,Math.round(header.x+header.w*.03));
    const panelRight=Math.min(full.width,Math.round(header.x+header.w*1.35));
    const panelTop=Math.max(0,Math.round(header.y+header.h*.95));
    const panelBottom=Math.min(full.height,Math.round(header.y+header.h*2.0));
    ...
    for(let y=panelTop;y<panelBottom;y+=2)for(let x=panelLeft;x<panelRight;x+=2){
      ...
      panelTotal++;
      if(hsv.v>=.68&&hsv.s<=.30)panelLight++;
    }
    const panelBackgroundRatio=panelTotal?panelLight/panelTotal:0;
    if(panelBackgroundRatio<.28)return{ready:false,reason:'weeklyPanelBackgroundNotFound',header,panelBackgroundRatio,components:components.slice(0,8),width:full.width,height:full.height};
    ...
    const size=Math.max(14,Math.min(34,Math.round(header.w*.10)));
...
async function boundImage(captureDir, entry) {
  if (!entry?.file || !entry?.sha256 || !entry?.mimeType) return null;
  const filePath = path.join(captureDir, String(entry.file));
  let bytes;
  try { bytes = await readFile(filePath); } catch { return null; }
  const mimeType = mimeFromBytes(bytes), digest = sha256(bytes);
  if (digest !== entry.sha256 || mimeType !== entry.mimeType) return null;
  return { file:String(entry.file), mimeType, captureSha256:digest, bytes };
}
...
export async function inspectWeeklyScreenshot({ captureDir, targetDate, repoRoot=path.resolve('.') }) {
  ...
  for(const media of (capture.rawMedia||[]).slice(0,4)) {
    const bound=await boundImage(captureDir,media);
    if(bound) sources.push({...bound,kind:'raw-media'});
  }
  const evidence=await boundImage(captureDir,capture.evidence);
  if(evidence) sources.push({...evidence,kind:'embed-screenshot'});
  if(!sources.length) throw new Error('weeklyEvidenceBindingMismatch');
  ...
  try{
    ...
    for(const source of sources){
      const inspected=await inspectScreenshot(page,`data:${source.mimeType};base64,${source.bytes.toString('base64')}`,templates);
      attempts.push({file:source.file,kind:source.kind,inspected});
      if(inspected.ready){selected={source,inspected};break;}
    }
  }finally{await browser.close();}
  if(!selected)return{ready:false,targetDate,selectedImage:null,diagnostics:{reason:'weeklyImageNotReady',attempts}};

  const {source,inspected}=selected;
  ...
  return{schemaVersion:1,ready:true,targetDate,selectedImage:{file:source.file,mimeType:source.mimeType,captureSha256:source.captureSha256},interpretation:{ready:true,baseDate:targetDate,baseDateDescription:`投稿本文/表示日時で${targetDate}を確認。`,days,confidence:'high',summary:'取得済みのゲーム内UI画像を優先して週間予報5日分を判読。',unresolved:[]},diagnostics:{mode:'weekly-image-first',selectedKind:source.kind,header:inspected.header,panelBackgroundRatio:inspected.panelBackgroundRatio,scores:inspected.days,attempts}};
}
