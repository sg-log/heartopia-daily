...
async function captureXEmbed({ post, outputDir }) {
  ...
  try {
    ...
    // X embeds may expose some pbs images but paint another photo inside the exact
    // status media link. Capture those exact-status rendered media as additional
    // candidates even when unrelated raw pbs images were already observed.
    const mediaLinks = frame.locator(`a[href*="/status/${post.sourceId}/photo/"], a[href*="/status/${post.sourceId}/video/"]`);
    const mediaLinkCount = Math.min(await mediaLinks.count(), 8);
    for (let index = 0; index < mediaLinkCount && rawMedia.length < 6; index += 1) {
      const target = mediaLinks.nth(index);
      const box = await target.boundingBox().catch(() => null);
      if (!box || box.width < 120 || box.height < 100) continue;
      await target.scrollIntoViewIfNeeded();
      const file = `raw-media-${rawMedia.length}.jpg`;
      const filePath = path.join(outputDir, file);
