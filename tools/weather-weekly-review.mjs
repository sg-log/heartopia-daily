...
function exactKeys(value, allowed, label) {
  ...
  if (keys.length !== expected.length || keys.some((key, i) => key !== expected[i])) throw new Error(`${label} has unexpected or missing properties.`);
}
...
function validateReviewImage(selected, index) {
  exactKeys(selected, ['role','file','mimeType','captureSha256','reviewStoredSha256','reviewUrl','expiresAt'], `selectedReviewImages[${index}]`);
  if (!['base-date','weekly-forecast'].includes(selected.role)) throw new Error(`selectedReviewImages[${index}] has an unsupported role.`);
  if (!/^raw-media-[0-3]\.(?:jpg|png)$/.test(selected.file)) throw new Error(`selectedReviewImages[${index}] must bind captured raw media.`);
  if (!['image/jpeg','image/png'].includes(selected.mimeType)) throw new Error(`selectedReviewImages[${index}] has an unsupported MIME type.`);
  if (!/^[a-f0-9]{64}$/.test(selected.captureSha256) || selected.reviewStoredSha256 !== selected.captureSha256) throw new Error(`selectedReviewImages[${index}] SHA-256 binding is invalid.`);
  if (!/^https:\/\/script\.google\.com\/macros\/s\/[A-Za-z0-9_-]+\/(?:exec|dev)\?reviewToken=[A-Za-z0-9_-]{43}$/.test(selected.reviewUrl)) throw new Error(`selectedReviewImages[${index}] private review URL is invalid.`);
  if (!/^20\d{2}-\d{2}-\d{2}T/.test(selected.expiresAt) || !Number.isFinite(Date.parse(selected.expiresAt))) throw new Error(`selectedReviewImages[${index}] expiry is invalid.`);
  if (Date.parse(selected.expiresAt) <= Date.now()) throw new Error(`selectedReviewImages[${index}] private review URL has expired.`);
  return { ...selected };
}

export function validateWeeklyReviewRecord(record) {
  exactKeys(record, ['schemaVersion','responseType','artifact','selectedReviewImages','interpretation'], 'review');
  ...
  for (const key of ['runId','id','name']) if (typeof record.artifact[key] !== 'string' || !record.artifact[key].trim()) throw new Error(`artifact.${key} is required.`);
  ...
  if (!Array.isArray(record.selectedReviewImages) || record.selectedReviewImages.length < 2 || record.selectedReviewImages.length > 4) throw new Error('Weekly review needs 2-4 bound review images/roles.');
  const selectedReviewImages = record.selectedReviewImages.map(validateReviewImage);
  if (!selectedReviewImages.some((item) => item.role === 'base-date')) throw new Error('A visually reviewed base-date image is required.');
  if (!selectedReviewImages.some((item) => item.role === 'weekly-forecast')) throw new Error('A visually reviewed weekly-forecast image is required.');

  const interpretation = record.interpretation;
  exactKeys(interpretation, ['ready','baseDate','baseDateDescription','days','confidence','summary','unresolved'], 'interpretation');
  ...
  if (typeof interpretation.baseDateDescription !== 'string' || !interpretation.baseDateDescription.trim() || interpretation.baseDateDescription.length > 160) throw new Error('baseDateDescription must state the visible date evidence.');
  ...
  if (!Array.isArray(interpretation.days) || interpretation.days.length < 5 || interpretation.days.length > 7) throw new Error('Five to seven consecutive weekly forecast days are required.');
  ...
  const normalizedDays = interpretation.days.map((day, index) => {
    ...
    const weather = day.weather.map(String);
  ...
  return {
    ...
    interpretation: {
      ...
      baseDateDescription: interpretation.baseDateDescription.trim(),
      days: normalizedDays,
      confidence: 'high',
...
function main() {
  ...
  const forecastImage = review.selectedReviewImages.find((item) => item.role === 'weekly-forecast');
  ...
  appendOutput('evidence_sha256', forecastImage.captureSha256);
}
