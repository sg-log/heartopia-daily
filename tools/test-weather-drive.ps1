...
assert(host.querySelectorAll('[data-load-weather-evidence]').length===1,'legacy no evidence button');
let apiCalls=0;
...
await loadPendingEvidence(host.querySelector('[data-load-weather-evidence]'));
